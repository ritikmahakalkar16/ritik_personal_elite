...
// const { z } = require('zod');
// const { addMonths, setDate, endOfMonth, isWithinInterval, startOfMonth } = require('date-fns');
// const fs = require('fs');
...
// const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://teerthaweb_db_user:9763767457@cluster0.vcl6nth.mongodb.net/?appName=Cluster0';
...
//   first_name: String,
//   last_name: String,
//   role: { type: String, enum: ['super_admin', 'manager', 'office_staff'], required: true },
...
//   approval_status: { type: String, enum: ['pending', 'approved', 'rejected', 'settled'], default: 'pending' },
//   submitted_by: String,
//   reviewed_by: String,
...
//   name: String,
//   segment: { type: String, enum: ['PRE-IPO', 'REAL ESTATE', 'DIRECT', 'INFRASTRUCTURE'] },
//   investment_amount: Number,
//   duration_months: Number,
//   return_percentage: Number,
...
//   investment_date: z.string().date().optional(),
...
// const InvestmentPaymentCreateSchema = z.object({
...
// async function auditLog(table_name, record_id, action, old_values, new_values, performed_by) {
//   try {
...
// async function generatePaymentSchedule(customer_id, investment_amount, investment_date, plan) {
//   const date = new Date(investment_date || new Date());
//   const day = date.getDate();
//   const isFirstHalf = day >= 1 && day <= 15;
//   const paymentDay = isFirstHalf ? 15 : 30;
//   const start_date = setDate(date, paymentDay);
//   const duration_months = plan.duration_months || 12;
//   const schedules = [];

//   if (plan.segment === 'INFRASTRUCTURE') {
//     const adjustedAmount = investment_amount * (1 - (plan.discount_percentage || 0) / 100);
//     if (plan.payment_type === 'Buyback') {
//       const payment_date = addMonths(start_date, duration_months).toISOString().split('T')[0];
//       schedules.push({
//         customer_id,
...
//       });
//     } else {
//       const monthlyAmount = adjustedAmount / duration_months;
//       for (let month = 1; month <= duration_months; month++) {
//         const payment_date = addMonths(start_date, month - 1).toISOString().split('T')[0];
//         schedules.push({
//           customer_id,
...
//           is_paid: false,
//           is_principal: month === duration_months,
//           principal_amount: month === duration_months ? adjustedAmount : 0,
...
//     }
//   } else {
//     const monthlyInterest = (investment_amount * (plan.return_percentage || 0) / 100) / 12;
//     if (plan.payment_type === 'Buyback') {
//       const totalReturn = investment_amount * (plan.return_percentage || 0) / 100 * duration_months / 12;
//       const payment_date = addMonths(start_date, duration_months).toISOString().split('T')[0];
//       schedules.push({
//         customer_id,
//         amount: investment_amount + totalReturn,
//         payment_date,
//         payment_type: 'Buyback',
//         is_paid: false,
//         is_principal: true,
...
//         const payment_date = addMonths(start_date, month - 1).toISOString().split('T')[0];
//         const isLastMonth = month === 13;
//         const amount = isLastMonth ? investment_amount + monthlyInterest : monthlyInterest;
//         schedules.push({
//           customer_id,
//           amount,
//           payment_date,
//           payment_type: 'Monthly',
//           is_paid: false,
//           is_principal: isLastMonth,
//           principal_amount: isLastMonth ? investment_amount : 0,
...
//   await auditLog('payment_schedules', customer_id, 'GENERATE_SCHEDULE', null, { schedules }, null);
//   return data;
// }

// async function generateInvestmentPayment(investment_id, investment_amount, plan, investment_date) {
...
//   const monthlyReturn = investment_amount * (plan.return_percentage || 0) / 100 / 12;
//   for (let month = 1; month <= duration_months; month++) {
//     const payment_date = addMonths(start_date, month - 1).toISOString().split('T')[0];
//     schedules.push({
//       investment_id,
...
//   const data = await InvestmentPayment.insertMany(schedules);
//   await auditLog('investment_payments', investment_id, 'GENERATE_SCHEDULE', null, { schedules }, null);
//   return data;
// }

// async function generateAgentPayments(customer_id, agent_id, investment_amount, approved_at) {
//   let currentAgent = await Agent.findById(agent_id);
//   const payments = [];
//   const payment_date = addMonths(new Date(approved_at), 1).toISOString().split('T')[0];

//   while (currentAgent) {
//     const commission = (investment_amount * currentAgent.commission_percentage) / 100;
//     payments.push({
...
//       const parentAgent = await Agent.findById(currentAgent.parent_agent_id);
//       if (parentAgent) {
//         const parentCommission = (investment_amount * (parentAgent.commission_percentage - currentAgent.commission_percentage)) / 100;
//         if (parentCommission > 0) {
//           payments.push({
//             agent_id: parentAgent._id,
//             customer_id,
...
//     await auditLog('agent_payments', customer_id, 'GENERATE_PAYMENTS', null, { payments }, null);
//     return data;
//   }
//   return [];
// }

// async function generateAgentRewards(customer_id, agent_id, investment_amount, approved_at) {
//   const performance_month = approved_at.toISOString().slice(0, 7); // YYYY-MM
//   let currentAgent = await Agent.findById(agent_id);
//   const rewards = [];

//   while (currentAgent) {
//     const giftPlans = await GiftPlan.find({ is_active: true });
//     for (const giftPlan of giftPlans) {
//       const customers = await Customer.find({
//         agent_id: currentAgent._id,
//         approval_status: 'approved',
//         approved_at: {
//           $gte: startOfMonth(new Date(performance_month)),
//           $lte: endOfMonth(new Date(performance_month))
//         }
//       });

//       const achieved_investors = customers.length;
//       const achieved_amount = customers.reduce((sum, c) => sum + c.investment_amount, 0);

//       if (achieved_investors >= giftPlan.target_investors || achieved_amount >= giftPlan.target_amount) {
//         rewards.push({
//           agent_id: currentAgent._id,
//           gift_plan_id: giftPlan._id,
//           performance_month,
//           achieved_investors,
//           achieved_amount,
//           is_rewarded: false,
...
//     currentAgent = currentAgent.parent_agent_id ? await Agent.findById(currentAgent.parent_agent_id) : null;
//   }

//   if (rewards.length > 0) {
//     const data = await AgentReward.insertMany(rewards);
//     await auditLog('agent_rewards', customer_id, 'GENERATE_REWARDS', null, { rewards }, null);
//     return data;
//   }
//   return [];
// }
...
// // Scheduled Deletion of Rejected Records
// cron.schedule('0 0 * * *', async () => {
//   const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
//   await Customer.deleteMany({ approval_status: 'rejected', updated_at: { $lte: twentyFourHoursAgo } });
//   await CompanyInvestment.deleteMany({ approval_status: 'rejected', updated_at: { $lte: twentyFourHoursAgo } });
//   await auditLog('cleanup', null, 'DELETE_REJECTED', null, { deleted: 'rejected records' }, null);
// });
...
//   try {
//     const adminExists = await Profile.findOne({ email: 'admin@elitewealth.com' });
//     if (!adminExists) {
//       await Profile.create({
//         user_id: uuidv4(),
//         email: 'admin@elitewealth.com',
//         first_name: 'Admin',
//         last_name: 'User',
//         role: 'super_admin',
//         active: true
...
//     if (plansCount === 0) {
//       const samplePlans = [
//         { name: 'PRE-IPO Fund', segment: 'PRE-IPO', investment_amount: 100000, duration_months: 12, return_percentage: 2, payment_type: 'Monthly', is_active: true },
//         { name: 'Real Estate Investment', segment: 'REAL ESTATE', investment_amount: 50000, duration_months: 12, return_percentage: 1.5, payment_type: 'Buyback', is_active: true },
//         { name: 'Direct Equity', segment: 'DIRECT', investment_amount: 200000, duration_months: 12, return_percentage: 3, payment_type: 'Monthly', is_active: true },
//         { name: 'Infrastructure Fund', segment: 'INFRASTRUCTURE', investment_amount: 150000, duration_months: 12, discount_percentage: 5, payment_type: 'Monthly', is_active: true }
//       ];
...
//     res.json({ data: { token, user: { user_id: user.user_id, email: user.email, role: user.role } }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'AUTH_ERROR', message: error.message } });
//   }
// });
...
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'PROFILE_ERROR', message: error.message } });
//   }
// });
...
// app.post('/profiles', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
//   try {
//     const validated = ProfileCreateSchema.parse(req.body);
//     const user_id = uuidv4();
//     const data = await Profile.create({ ...validated, user_id, created_at: new Date(), updated_at: new Date() });
//     await auditLog('profiles', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
...
//   try {
//     if (req.user.role !== 'super_admin' && req.params.user_id !== req.user.user_id) {
...
//       { new: true }
//     );
//     await auditLog('profiles', data._id, 'UPDATE', oldData?.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
//   try {
//     const { page = 1, page_size = 20, status, plan_id, agent_id, search } = req.query;
//     const query = {};
...
//     res.json({ data: { items: data, total }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'CUSTOMER_ERROR', message: error.message } });
//   }
// });

// app.post('/customers', authMiddleware, rbacMiddleware(['office_staff', 'manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const validated = CustomerCreateSchema.parse(JSON.parse(req.body.data || '{}'));
...
//       created_at: new Date(),
//       updated_at: new Date(),
//       investment_date: validated.investment_date || new Date().toISOString().split('T')[0]
//     });
//     await auditLog('customers', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// app.post('/customers/:id/approve', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
...
//     if (plan) {
//       await generatePaymentSchedule(data._id, data.investment_amount, data.investment_date || data.created_at, plan);
//       if (data.agent_id) {
//         await generateAgentPayments(data._id, data.agent_id, data.investment_amount, data.approved_at);
//         await generateAgentRewards(data._id, data.agent_id, data.investment_amount, data.approved_at);
//       }
//     }

//     await auditLog('customers', data._id, 'APPROVE', oldData?.toObject(), data.toObject(), req.user.user_id);
...
// app.post('/customers/:id/reject', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const { comments } = req.body;
//     const oldData = await Customer.findById(req.params.id);
//     if (!oldData) throw new Error('Customer not found');
...
//       { new: true }
//     );
//     await auditLog('customers', data._id, 'REJECT', oldData?.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data: { success: true, updated: data }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'APPROVAL_ERROR', message: error.message } });
//   }
...
// app.patch('/customers/:id', authMiddleware, rbacMiddleware(['office_staff', 'manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const validated = CustomerCreateSchema.partial().parse(JSON.parse(req.body.data || '{}'));
//     const oldData = await Customer.findById(req.params.id);
//     if (!oldData) throw new Error('Customer not found');
...
//       nominee_adhar_pan_number: validated.nominee_adhar_pan_number ?? oldData.nominee_adhar_pan_number,
//       email: validated.email ?? oldData.email,
//       phone: validated.phone ?? oldData.phone,
//       updated_at: new Date()
//     };
//     const data = await Customer.findByIdAndUpdate(req.params.id, updateData, { new: true });
//     await auditLog('customers', data._id, 'UPDATE', oldData.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// app.get('/customers/export', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
...
//       { header: 'Aadhar Number', key: 'aadhar_number' },
//       { header: 'Plan ID', key: 'plan_id' },
//       { header: 'Investment Amount', key: 'investment_amount' },
//       { header: 'Investment Date', key: 'investment_date' },
//       { header: 'Nominee', key: 'nominee' },
//       { header: 'Nominee Aadhar/PAN', key: 'nominee_adhar_pan_number' },
...
//     ];
//     const { buffer, filename } = await exportToExcel(customers, columns, 'customers.xlsx');
...
//   try {
//     const { page = 1, page_size = 20, status, plan_id, search } = req.query;
//     const query = {};
//     if (status) query.approval_status = status;
//     if (plan_id) query.plan_id = plan_id;
...
//     res.json({ data: { items: data, total }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'INVESTMENT_ERROR', message: error.message } });
//   }
// });

// app.post('/investments', authMiddleware, rbacMiddleware(['office_staff', 'manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const validated = CompanyInvestmentCreateSchema.parse(JSON.parse(req.body.data || '{}'));
//     const images = req.files ? await uploadImages(req.files) : [];
//     let { investment_amount, expected_return, return_percentage, duration_months } = validated;

//     if (expected_return && !return_percentage) {
//       return_percentage = (expected_return / investment_amount) * 100 * 12 / duration_months;
//     } else if (return_percentage && !expected_return) {
//       expected_return = investment_amount * (return_percentage / 100) * duration_months / 12;
//     }
...
//       updated_at: new Date()
//     });
//     await auditLog('company_investments', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// app.post('/investments/:id/approve', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const { comments } = req.body;
...
//     const plan = await Plan.findById(data.plan_id);
//     if (plan) {
//       await generateInvestmentPayment(data._id, data.investment_amount, plan, data.investment_date);
//     }
//     await auditLog('company_investments', data._id, 'APPROVE', oldData?.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data: { success: true, updated: data }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'APPROVAL_ERROR', message: error.message } });
//   }
...
// app.post('/investments/:id/reject', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const { comments } = req.body;
//     const oldData = await CompanyInvestment.findById(req.params.id);
//     if (!oldData) throw new Error('Investment not found');
...
//       { new: true }
//     );
//     await auditLog('company_investments', data._id, 'REJECT', oldData?.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data: { success: true, updated: data }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'APPROVAL_ERROR', message: error.message } });
//   }
...
// app.patch('/investments/:id', authMiddleware, rbacMiddleware(['office_staff', 'manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const validated = CompanyInvestmentCreateSchema.partial().parse(JSON.parse(req.body.data || '{}'));
//     const oldData = await CompanyInvestment.findById(req.params.id);
//     if (!oldData) throw new Error('Investment not found');
//     let { investment_amount, expected_return, return_percentage, duration_months } = { ...oldData.toObject(), ...validated };
//     if (expected_return && !return_percentage) {
//       return_percentage = (expected_return / investment_amount) * 100 * 12 / duration_months;
//     } else if (return_percentage && !expected_return) {
//       expected_return = investment_amount * (return_percentage / 100) * duration_months / 12;
...
//     const newImages = req.files ? await uploadImages(req.files) : [];
//     const images = [...(oldData.images || []), ...newImages];
//     const updateData = { ...validated, expected_return, return_percentage, images, updated_at: new Date() };
//     const data = await CompanyInvestment.findByIdAndUpdate(req.params.id, updateData, { new: true });
//     await auditLog('company_investments', data._id, 'UPDATE', oldData.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
//   try {
//     const query = req.query.is_active === 'true' ? { is_active: true } : {};
//     const data = await Plan.find(query).sort({ created_at: -1 });
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'PLAN_ERROR', message: error.message } });
//   }
// });

// app.post('/plans', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const validated = PlanCreateSchema.parse(req.body);
//     const data = await Plan.create({ ...validated, created_at: new Date(), updated_at: new Date(), created_by: req.user.user_id });
//     await auditLog('plans', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// app.patch('/plans/:id', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const oldData = await Plan.findById(req.params.id);
//     const data = await Plan.findByIdAndUpdate(req.params.id, { ...req.body, updated_at: new Date() }, { new: true });
//     if (!data) throw new Error('Plan not found');
//     await auditLog('plans', data._id, 'UPDATE', oldData?.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
//   try {
//     const { page = 1, page_size = 20, customer_id, status } = req.query;
//     const query = {};
...
//     res.json({ data: { items: data, total }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'PAYMENT_SCHEDULE_ERROR', message: error.message } });
//   }
// });

// app.post('/payment_schedules/generate', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const { customer_id, investment_amount, investment_date, plan_id } = req.body;
//     const plan = await Plan.findById(plan_id);
//     if (!plan) throw new Error('Plan not found');
//     const schedules = await generatePaymentSchedule(customer_id, investment_amount, investment_date, plan);
//     res.json({ data: schedules, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'GENERATION_ERROR', message: error.message } });
//   }
// });

// app.patch('/payment_schedules/:id/mark_paid', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const oldData = await PaymentSchedule.findById(req.params.id);
//     if (!oldData) throw new Error('Payment schedule not found');
//     const data = await PaymentSchedule.findByIdAndUpdate(
//       req.params.id,
//       { is_paid: true, paid_at: new Date() },
//       { new: true }
//     );
//     await auditLog('payment_schedules', data._id, 'PAYMENT_MARKED', oldData?.toObject(), data.toObject(), req.user.user_id);

//     // Check if customer should be marked as settled
//     const schedules = await PaymentSchedule.find({ customer_id: oldData.customer_id });
//     const allPaid = schedules.every(s => s.is_paid);
//     if (allPaid) {
//       await Customer.findByIdAndUpdate(oldData.customer_id, { approval_status: 'settled', updated_at: new Date() });
//       await auditLog('customers', oldData.customer_id, 'SETTLED', null, { approval_status: 'settled' }, req.user.user_id);
//     }

//     res.json({ data: { success: true, updated: data }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'PAYMENT_ERROR', message: error.message } });
//   }
// });

// // Agent Payments Routes
// app.get('/agent-payments', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
...
//     res.json({ data: { items: data, total }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'AGENT_PAYMENT_ERROR', message: error.message } });
//   }
// });

// app.post('/agent-payments', authMiddleware, rbacMiddleware(['manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const validated = AgentPaymentCreateSchema.parse(JSON.parse(req.body.data || '{}'));
//     const images = req.files ? await uploadImages(req.files) : [];
...
//       created_at: new Date()
//     });
//     await auditLog('agent_payments', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// // Investment Payments Routes
// app.get('/investment-payments', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
//   try {
...
//     res.json({ data: { items: data, total }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'INVESTMENT_PAYMENT_ERROR', message: error.message } });
//   }
// });

// app.post('/investment-payments', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
//   try {
//     const validated = InvestmentPaymentCreateSchema.parse(req.body);
//     const data = await InvestmentPayment.create({ ...validated, created_at: new Date() });
//     await auditLog('investment_payments', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// // Gift Plans Routes
// app.get('/gifts', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
//   try {
//     const query = req.query.is_active === 'true' ? { is_active: true } : {};
//     const data = await GiftPlan.find(query).sort({ created_at: -1 });
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'GIFT_PLAN_ERROR', message: error.message } });
//   }
// });

// app.post('/gifts', authMiddleware, rbacMiddleware(['super_admin']), async (req, res) => {
//   try {
//     const validated = GiftPlanCreateSchema.parse(req.body);
//     const data = await GiftPlan.create({ ...validated, created_at: new Date(), updated_at: new Date() });
//     await auditLog('gift_plans', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// // Agent Rewards Routes
// app.get('/gift-payments', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const { page = 1, page_size = 20, agent_id } = req.query;
//     const query = agent_id ? { agent_id } : {};
//     const total = await AgentReward.countDocuments(query);
//     const data = await AgentReward.find(query)
//       .skip((page - 1) * page_size)
//       .limit(Math.min(page_size, 100))
//       .sort({ performance_month: -1 });
//     res.json({ data: { items: data, total }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'AGENT_REWARD_ERROR', message: error.message } });
//   }
// });

// app.post('/gift-payments', authMiddleware, rbacMiddleware(['manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const validated = AgentRewardCreateSchema.parse(JSON.parse(req.body.data || '{}'));
//     const images = req.files ? await uploadImages(req.files) : [];
//     const data = await AgentReward.create({
//       ...validated,
//       images,
//       created_at: new Date()
//     });
//     await auditLog('agent_rewards', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
//     res.json({ data, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'VALIDATION_ERROR', message: error.message } });
//   }
...
// app.get('/gift-payments/export', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const rewards = await AgentReward.find().lean();
//     const columns = [
//       { header: 'ID', key: '_id' },
//       { header: 'Agent ID', key: 'agent_id' },
//       { header: 'Gift Plan ID', key: 'gift_plan_id' },
...
//       { header: 'Is Rewarded', key: 'is_rewarded' },
//       { header: 'Rewarded At', key: 'rewarded_at' },
//       { header: 'Reward Method', key: 'reward_method' },
//       { header: 'Transaction ID', key: 'transaction_id' },
//       { header: 'Created At', key: 'created_at' }
//     ];
//     const { buffer, filename } = await exportToExcel(rewards, columns, 'agent_rewards.xlsx');
//     res.setHeader('Content-Disposition', `attachment; filename=${filename}`);
//     res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
//     res.send(buffer);
//   } catch (error) {
...
//     });
//   } catch (error) {
//     res.status(500).json({ data: null, error: { code: 'STATS_ERROR', message: error.message } });
//   }
// });
...
//   console.error(error);
//   res.status(500).json({ data: null, error: { code: 'INTERNAL_ERROR', message: error.message } });
// });
...
//   pan_number: String,
//   aadhar_number: String,
//   plan_id: String, // REQUIRED FOR CUSTOMERS
//   investment_amount: Number,
//   investment_date: String,
//   nominee: String,
//   nominee_adhar_pan_number: String,
...
// async function generateAgentRewards(customer_id, agent_id, investment_amount, approved_at) {
//   const performance_month = approved_at.toISOString().slice(0, 7);
//   let currentAgent = await Agent.findById(agent_id);
//   const rewards = [];
...
// // AUTO-GENERATE INVESTMENT PAYMENT ON APPROVAL
// async function generateInvestmentPaymentOnApproval(investment) {
//   const start = new Date(investment.investment_date);
//   const paymentDate = addMonths(start, investment.duration_months).toISOString().split('T')[0];

//   let profit = 0;
//   if (investment.return_percentage != null) {
//     profit = investment.investment_amount * (investment.return_percentage / 100) * (investment.duration_months / 12);
//   } else if (investment.expected_return != null) {
//     profit = investment.expected_return;
//   }

//   const finalAmount = investment.investment_amount + profit;
...
//     null,
//     { investment_amount: investment.investment_amount, profit, final_amount: finalAmount, payment_date: paymentDate },
//     'system'
//   );
...
//         { name: 'Real Estate Investment', segment: 'REAL ESTATE', investment_amount: 50000, duration_months: 12, return_percentage: 1.5, payment_type: 'Buyback', is_active: true },
//         { name: 'Direct Equity', segment: 'DIRECT', investment_amount: 200000, duration_months: 12, return_percentage: 3, payment_type: 'Monthly', is_active: true },
//         { name: 'Infrastructure Fund', segment: 'INFRASTRUCTURE', duration_months: 12, discount_percentage: 5, payment_type: 'Monthly', is_active: true }
//       ];//removed from segment infrastructure fund investment_amount: 200000
//       await Plan.insertMany(samplePlans);
//       console.log('Seeded sample plans');
//     }
//   } catch (error) {
...
// // Helper for return calculation
// function calculateReturnFields({ investment_amount, expected_return, return_percentage, duration_months }) {
//   if (expected_return && !return_percentage) {
//     return_percentage = (expected_return / investment_amount) * 100 * 12 / duration_months;
//   } else if (return_percentage && !expected_return) {
//     expected_return = investment_amount * (return_percentage / 100) * duration_months / 12;
...
//     // AUTO-GENERATE PAYMENT: Principal + Profit
...
// app.get('/investments/export', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
...
//     ];
//     const { buffer, filename } = await exportToExcel(investments, columns, 'investments.xlsx');
//     res.setHeader('Content-Disposition', `attachment; filename=${filename}`);
//     res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
//     res.send(buffer);
//   } catch (error) {
...
//       const inv = await CompanyInvestment.findById(p.investment_id).select('investment_name investment_amount');
//       return {
//         ...p.toObject(),
...
// // Agent Payments, Gift Plans, etc. (unchanged)
...
// app.post('/gift-plans', authMiddleware, rbacMiddleware(['manager', 'super_admin']), async (req, res) => {
//   try {
//     const validated = GiftPlanCreateSchema.parse(req.body);
//     const data = await GiftPlan.create({ ...validated, created_at: new Date(), updated_at: new Date() });
//     await auditLog('gift_plans', data._id, 'CREATE', null, data.toObject(), req.user.user_id);
...
// app.patch('/agent-rewards/:id/mark_rewarded', authMiddleware, rbacMiddleware(['manager', 'super_admin']), upload.array('files'), async (req, res) => {
//   try {
//     const { reward_method, transaction_id } = req.body;
//     const images = req.files ? await uploadImages(req.files) : [];
//     const oldData = await AgentReward.findById(req.params.id);
//     if (!oldData) throw new Error('Reward not found');
//     const data = await AgentReward.findByIdAndUpdate(
//       req.params.id,
//       { is_rewarded: true, rewarded_at: new Date(), reward_method, transaction_id, images },
//       { new: true }
//     );
//     await auditLog('agent_rewards', data._id, 'REWARD_MARKED', oldData?.toObject(), data.toObject(), req.user.user_id);
//     res.json({ data: { success: true, updated: data }, error: null });
//   } catch (error) {
//     res.status(400).json({ data: null, error: { code: 'REWARD_ERROR', message: error.message } });
//   }
// });
